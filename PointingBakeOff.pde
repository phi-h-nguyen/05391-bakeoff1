...
int row = -1;
float rowSize = (buttonSize * 2);
float halfRowSize = rowSize / 2;
int[] rowMidYs = {220, 310, 400, 490};
...
void drawButton(int i) {
  ...
  if (row == y) {
    fill(0,0,0);
    text((x + 1), bounds.x + buttonSize / 2, bounds.y + buttonSize / 2);
  }
}

void drawRowHighlight() {
  if (row >= 0) {
    float mid = float(rowMidYs[row]);
    fill(100, 100, 100);
    rect(150,(mid - halfRowSize), 410, rowSize);
  } 
}

void mouseMoved() { 
  int tmp = -1;
  for (int i = 0; i < rowMidYs.length; i++) {
    if (mouseY > (rowMidYs[i] - halfRowSize) && mouseY < rowMidYs[i] + halfRowSize) {
      tmp = i;
    }
  }
  row = tmp;
...
void keyPressed() {
  ...
  int[] keys = {49, 50, 51, 52};
  if (row >= 0) {
    int x = trials.get(trialNum) % 4;
    int y = trials.get(trialNum) / 4;
    if (y == row) {
      if (int(key) == keys[x]) {
